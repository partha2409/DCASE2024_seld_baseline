...
def main(argv):
    ...
    if params['modality'] == 'audio_visual':
        data_in, vid_feat_in, data_out = data_gen_test.get_data_sizes()
        model = seldnet_model.SeldModel(data_in, data_out, params, vid_feat_in)
    else:
    ...
    with torch.no_grad():
        file_cnt = 0
        for values in data_gen_test.generate():
            if len(values) == 2:
                data, target = values
